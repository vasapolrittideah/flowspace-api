...
| ID-T03 | Guessing or flooding six-digit verification and recovery codes. Spoofing and denial of service. | Bind each code to one subject, purpose, and destination. Store only a protected verifier. Set a short expiry, attempt limit, resend limit, and issue limit. Invalidate the code after success and when its replacement becomes active. | Boundary tests cover expiry, wrong purpose, replay, replacement, attempt exhaustion, and concurrent submissions. Email tests prove that one caller cannot cause unbounded delivery. |
