...
- End `Reason` with `Closes #<issue-number>.` for each Issue that the PR completes. Leave one blank line before the first `Closes` line. GitHub closes those Issues after the PR merges into `main`.
