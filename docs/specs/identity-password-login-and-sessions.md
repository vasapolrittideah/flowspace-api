...
The access token is an Ed25519-signed JWT with type `at+jwt` and audience `flowspace-api`. Its issuer is `flowspace-identity-local`, `flowspace-identity-staging`, or `flowspace-identity-production` for the matching environment. It carries a stable subject, session ID, issue time, expiry, token ID, and known key ID. A verifier rejects a missing or invalid claim, a different algorithm, issuer, audience, or type, an unknown key ID, a bad signature, an expired token, or a token whose `exp - iat` exceeds ten minutes. It permits at most 30 seconds of future `iat` skew and no skew beyond `exp`.

Identity publishes public verification keys at `GET /.well-known/jwks.json` with a 60-second maximum cache age. Only Identity uses private keys. Protected services retrieve JWKS from a configured internal HTTPS address with a pinned trust root, refresh on unknown key IDs at most once per five seconds per verifier, and deny new protected requests if an expired cache cannot be refreshed. They check live session state on every protected request. JWKS never contains a private key or refresh-token material.
...
- Login checks request shape and shared rate limits before password hashing. It permits at most 30 requests per source IP in ten minutes and ten requests per normalized email identifier in 15 minutes, using separate PostgreSQL-backed limits. Missing accounts consume both limits, and stored email limit keys are keyed digests. Identity uses the peer IP unless a deployment names trusted proxy peers and makes the edge replace client-supplied forwarding headers. If limit state is unavailable, Identity returns `Unavailable` and does not bypass the limit.
...
- Identity uses server time for every expiry decision. At the exact expiry instant, the token or session is expired. The 30-second future `iat` tolerance does not extend access-token or server-side session expiry.
...
- Identity keeps its encrypted signing keyring on an Identity-only persistent volume and obtains the decryption key from a workload-scoped Kubernetes Secret. Protected services cannot read the signing keys. A missing or unreadable keyring stops token issuance.
- Identity rotates signing keys automatically every 24 hours. It publishes a replacement public key at least two minutes before signing with it and retains the old public key for at least 11 minutes after its last signature. For suspected key theft, Identity stops issuance, revokes every session, removes the suspect public key, publishes a fresh key, and waits at least two minutes before issuing new sessions. If revocation cannot commit, issuance stays stopped.
...
- Obtain approval before changing the accepted security profile in ADR-0035, including signing, rotation, JWKS, internal authentication, proxy trust, or numeric login limits.
