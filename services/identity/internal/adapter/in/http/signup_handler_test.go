...
func TestSignupHandler(t *testing.T) {
	...
	handler := identityhttp.NewIdentityHandler(service, nil, nil, nil, nil, nil)
...
func TestGeneratedSignupRESTRoute(t *testing.T) {
	...
	if err := identityv1.RegisterIdentityServiceHandlerServer(context.Background(), mux, identityhttp.NewIdentityHandler(service, nil, nil, nil, nil, nil)); err != nil {
		t.Fatal(err)
	}
