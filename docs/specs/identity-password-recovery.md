...
`RequestPasswordResetCode` returns the same `accepted=true` response for a missing account, an unverified account, a provider-only account, and an eligible account. It sends email only for an eligible account. A repeat request for the same email within 60 seconds returns `accepted=true` without issuing another code if the earlier request committed. If it did not commit, the retry can issue the first code. Responses and practical timing must not reveal eligibility. A malformed email returns `InvalidArgument` (HTTP 400) without revealing account state.
...
- The challenge expires ten minutes after issue and can succeed once. Identity issues a new code only when at least 60 seconds have passed since the previous request. It issues no more than five codes per account per hour across verification, claim, and reset requests. A repeat request within 60 seconds counts toward the source-address request limit but does not replace the current challenge. Source-address and account limits use shared state across replicas. The same public limit behavior applies to missing and ineligible accounts.
...
Public REST and typed RPC tests cover a missing, unverified, verified password, linked-provider password, and provider-only account. Compare generic request responses, practical timing, rate-limit behavior, and invalid-code errors for eligible and ineligible accounts. Prove that a repeat code request within 60 seconds returns `accepted=true` without replacing a committed challenge, and that a retry after an uncommitted request can issue the first code. Exercise malformed input, wrong-purpose codes, an older email arriving late, failed delivery, cancellation, and a lost reset response. Mailpit and cross-service tests prove that reset sends the code and notice, old sessions fail immediately after commit, and login with the new password creates a fresh session.
